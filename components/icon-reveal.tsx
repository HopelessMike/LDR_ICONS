  ...
  useEffect(() => {
    if (!isAnalyzing && !show) {
      const scheduleGlitch = () => {
        // Random delay between 15-45 seconds, staggered by slot index
        const baseDelay = 15000 + Math.random() * 30000;
        const slotStagger = slotIndex * 3000; // 3 second stagger per slot
        const totalDelay = baseDelay + slotStagger;
        
        const timeoutId = setTimeout(() => {
          setGlitchType(Math.random() > 0.7 ? 'intense' : 'normal');
          setIndividualGlitchActive(true);
          
          // Glitch lasts for 200ms
          setTimeout(() => {
  ...
  useEffect(() => {
    if (isLocked) {
      const scheduleLockedGlitch = () => {
        // Even less frequent for locked icons: 30-90 seconds
      ...
      >
        <IconComponent
          className={cn(
            ...
            isLocked && individualGlitchActive && "animate-idle-glitch", // Glitch for locked icons
